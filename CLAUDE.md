...
- **`ecosystem.config.js`** — config PM2 (nom du process + script)
...
- **`tests/cleaner-logic.test.js`** — 27 tests sur `shouldDelete` (toutes les branches logiques)
- **`tests/grab-logic.test.js`** — 36 tests sur `filterCandidates` (filtres, tri, limites)
...
Un seul timer combiné `scheduleTimer()` dans `server.js` — un `setInterval` toutes les 60s qui vérifie si `cfg.timer.interval_hours` est écoulé depuis `cfg.timer.last_run`. Ce mécanisme (elapsed-time check) permet des intervalles arbitraires sans dépendre d'une lib cron.

Séquence à chaque déclenchement :
1. `cleaner.runClean('auto')` — si `cfg.auto_clean.enabled`
2. Pause 10 secondes — laisse qBittorrent finaliser les suppressions
3. `grab.runAutoGrab('auto')` — si `cfg.auto_grab.enabled`
