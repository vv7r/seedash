...
async function initAuth(saveConn) {
  let changed = false;
  if (!_cfg.auth) _cfg.auth = { username: 'admin', password_hash: '', jwt_secret: '', token_expiry: '24h', issued_after: 0, setup_completed: false };
  ...
  if (!_cfg.auth.password_hash) {
    ...
    if (_cfg.auth.setup_completed === undefined) _cfg.auth.setup_completed = false;
    changed = true;
  }
