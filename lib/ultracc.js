...
async function getUltraccStats() {
  ...
  try {
    ...
    ultraccCache.lastError = status === 404
      ? `404 sur ${_cfg.ultracc_api.url} — selon la version du script Ultra-API, la route est /total-stats ou /total_stats`
      : status === 502 || status === 503
        ? `${status} — le service Ultra-API ne répond pas ; vérifiez le screen UltraAPIpoints (screen -ls)`
