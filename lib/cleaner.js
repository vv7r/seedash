...
const LOG_PATH            = path.join(ROOT, 'logs', 'cleaner.log');
  ...
  try {
    ...
    for (const t of toDelete) {
      try {
        ...
        deleted++;
        deletedNames.push({ name: t.name, url: `${(cfg.c411?.url || '').replace('/api/torznab', '') || 'https://c411.org'}/torrents/${t.hash}` });
...
function init() {
  ...
  // Le timer est géré par server.js (scheduleTimer) — pas de reschedule ici
}
