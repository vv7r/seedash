...
async function runAutoGrab(source = 'auto') {
  ...
  try {
    ...
    const { network_max_pct } = _cfg.auto_grab.rules || {};
    if (isRuleOn('network_max_pct') && network_max_pct != null && network_max_pct > 0 && _fns.getUltraccInfo) {
      try {
        const info = await _fns.getUltraccInfo();
        if (info?.traffic_used_pct != null && info.traffic_used_pct >= network_max_pct) {
          console.log(`[auto-grab] Trafic réseau ${info.traffic_used_pct}% ≥ ${network_max_pct}% — grab annulé`);
          return 0;
        }
      } catch(e) {
        console.error('[auto-grab] Erreur vérif. réseau :', e.message);
        // Non bloquant si Ultra.cc inaccessible
      }
    }
    ...
    if (candidates.length > 0 && _fns.getUltraccInfo) {
      try {
        const info = await _fns.getUltraccInfo();
        if (info?.free_storage_gb != null) {
          const totalBytes = candidates.reduce((s, t) => s + (t.size || 0), 0);
          if (totalBytes > info.free_storage_gb * 1e9) {
            console.log(`[auto-grab] Espace insuffisant : ${(totalBytes / 1e9).toFixed(1)} GB requis, ${info.free_storage_gb.toFixed(1)} GB libre`);
            return 0;
          }
        }
      } catch(e) {
        console.error('[auto-grab] Erreur vérif. disque :', e.message);
        // Non bloquant si Ultra.cc inaccessible
      }
    }
