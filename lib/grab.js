  ...
  try {
    ...
    if (_fns.getUltraccInfo) {
      try {
        ...
        if (!allowed) { console.log(`[auto-grab] ${reason} — grab annulé`); return 0; }
      } catch(e) { console.error('[auto-grab] Erreur vérif. réseau :', e.message); }
    }
    ...
    if (candidates.length > 0 && _fns.getUltraccInfo) {
      try {
        ...
        if (!allowed) { console.log(`[auto-grab] ${reason} — grab annulé`); return 0; }
      } catch(e) { console.error('[auto-grab] Erreur vérif. disque :', e.message); }
    }
...
module.exports = { init, runAutoGrab, filterCandidates, checkGrabConditions, scheduleAutoGrab, getStatus, setEnabled };
